...
token = '6293017023:AAG0xUXcNK0xyWRlF8T4xo4asYBIHJvJyqA'
...
def phone(message):
 ...
 if kl == None:
  if glob.glob(f'H/H{message.from_user.id}.txt'):
   ...
   a1 = types.InlineKeyboardButton(text="- اربيل .",callback_data='erbil;')
   ...
   a4= types.InlineKeyboardButton(text="- بابل .",callback_data="babylon;")
   ...
   a10 = types.InlineKeyboardButton(text="- صلاح الدين .",callback_data='balad;')
   ...
   a13 = types.InlineKeyboardButton(text="- كركوك .",callback_data="kirkuk;")
   a14 = types.InlineKeyboardButton(text="- المثنى .",callback_data="muthana;")
 ...
 - اضغط على الزر ادناه لكي اتحقق ذلك ...''', reply_markup = keyboard)
   ...
   bot.reply_to(message,'- تم اعطائك محاولتين لزيادة المحاولات راسل المطور @LuMaX ')
       ...
       if int(de(open(f'H/H{call.from_user.id}.txt','r').read())) >= 1:
        ...
        if str(call.message.chat.id) in u or str(call.message.chat.id) == id:
         ...
         if ';' in call.data:
          ...
          h = bot.send_message(call.message.chat.id,'''("يرجى ادخال البيانات بهذا الشكل!")
(جسني ايهاب هادي 2009) 'بدون اقواس'
- "وتقدر ترسل بدون مواليد"
(جسني ايهاب هادي) 'بدون اقواس'
- "مع دمج الاسامي المركبة"  مثل:
عبد العزيز (يصير عبدالعزيز)
زين العابدين (يصير زينالعابدين)
- "وتقدر ترسل بس الجزء الاول من الاسم المركب" مثل :
زينالعابدين (زين)
عبدالعزيز (عبد)
==============================
للمساعدة يرجى التواصل مع المطور : @LuMaX
استمتعوا (؛''')
...
def huks(message, place):
 ...
 محل الولادة : {V}
      ...
      markup.add(btn_get_info)
      bot.send_message(message.chat.id, all, reply_markup=markup)
 if h == 0:bot.reply_to(message,'- حبيبي تأكد من معلوماتك او من المواليد او جرب ترسل الاسم هكذا \n- جسني ايهاب هادي \n- ركز بدون مواليد !! \n-  للمساعدة : @LuMaX')
